...
class Trainer(object):
    ...
    def train_batch(self, xy):
        observed = xy[:5]
        prediction_truth = xy[5:-1].clone()  ## CLONE
        targets = xy[5:, 0] - xy[4:-1, 0]

        self.optimizer.zero_grad()
        rel_outputs, outputs = self.model(observed, prediction_truth)

        ## Loss wrt primary only
        loss = self.criterion(rel_outputs[-5:, 0], targets) * 100
        loss.backward()
...
def main(epochs=50):
    ...
    # these2 following lines is for loading ndjson data from correspondent Data_BLOCK/trajdata/trian
